...
const (
	...
	arriveDistance = 0.05 // close enough to TargetX/TargetY counts as arrived
)
...
// update moves the unit toward its target by at most unitSpeed*dt, clearing
// HasTarget once it arrives.
...
// into HandleCommand before each Tick, so this rule still holds.
type World struct {
	TickCount int64
	Units     []*Unit
}
...
// the Phase 1 "Hello Tank" milestone. Owner is hardcoded to 1 — there's
// only one player until Phase 6 adds real multiplayer identity.
func NewWorld() *World {
	return &World{
		Units: []*Unit{
			{ID: 1, X: 0, Y: 0, Owner: 1},
		},
	}
}
...
// HandleCommand applies a move command to every unit it names that the
// command's owner actually controls. Units not owned by cmd.Owner, or not
// present in the world, are silently ignored.
