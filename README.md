...
地图是 ASCII 数据(见 [server/game/maps.go](server/game/maps.go)),一个字符一格:

```
.  草地            =  公路（可通行，纯装饰）
~  水（不可通行）   #  悬崖（不可通行）
*  矿柱 —— 不可通行，周围会长矿
1  玩家 1 的基地位置    2  玩家 2 的
```

**第一行是地图顶部**,所以源码里读起来就是俯视图(解析时会翻转,因为世界的 y 向上增长)。

这么做不只是为了看得懂。出生点和矿柱以前是 `NewWorld` 里的魔数,和地形是**两处独立的硬编码**,
改地图就得手动对齐;现在它们是同一份数据,加一张新图只要画一张 ASCII。

现有两张 40×40,创建房间时选:

| 名字 | 布局 |
|---|---|
| `crossroads` | 双方在对角,中央十字路口,两侧湖泊夹着纵向公路 |
| `ridge` | 双方在左右两端,中间一道纵向山脊,只有两个缺口 |

两张都是**旋转对称**的——否则等于凭空送给某一方地形优势。有测试钉住这一点,
以及"每张图 AI 都能正常打"(见 `maps_test.go`)。
